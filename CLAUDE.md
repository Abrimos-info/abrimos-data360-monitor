...
bin/
  ...
  fetch-news.js          # Headline pool (Gemini/GDELT per country) → data/news/{COUNTRY}/{YYYY-MM}.jsonl
...
| D-030 | News subsystem: headlines at `data/news/{COUNTRY}/{YYYY-MM}.jsonl`; **pool mode** default (`lib/news-pool.js`): ≤5 Gemini calls per country with GDELT fallback, skip when covered; legacy per-indicator mode available. Max 8 headlines per country in reportaje context, filtered by `--as-of`. Passive narrative context only. Full design at `docs/news-architecture.md`. |
