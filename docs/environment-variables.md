...
## Titulares — GDELT y Gemini

| Variable | Propósito | Default | Clave API |
|----------|-----------|---------|-----------|
| `GEMINI_API_KEY` | Si está definida, `fetch-news` usa Gemini; si no, GDELT | *(vacío → GDELT)* | [Google AI Studio](https://aistudio.google.com/apikey) |
...
| `GEMINI_SKIP_COVERED` | `false`: vuelve a consultar indicadores ya cubiertos | `true` | — |
| `GEMINI_NEWS_MIN_ACCEPTED` | Mínimo titulares aceptados por indicador | `1` | — |
| `GEMINI_NEWS_BATCH` | `false`: una llamada Gemini por país (no batch LAC) | `true` | — |
