# Corridor lead time from port exit to destination (LLDC)
...
- **csv**: https://data360files.worldbank.org/data360-data/data/WB_LPI_20/LT_CORR.csv
- **json metadata**: https://data360files.worldbank.org/data360-data/metadata/WB_LPI_20/LT_CORR.json
- **api template**: https://data360api.worldbank.org/data360/data?top=1000&skip=0&DATABASE_ID=WB_LPI_20&INDICATOR=LT_CORR
- **dataset on Data360**: https://data360.worldbank.org/en/search?query=WB_LPI_20

## Definition

Time spent by containers from port of arrival to the clearance facility in the destination LLDCs.
...
Corridor lead time is calculated as the time difference between the moment a container exits the port of entry (port of arrival) of the transit coastal economy until the arrival time at the destination location in the landlocked country.
