...
- **csv**: https://data360files.worldbank.org/data360-data/data/WB_LPI_20/CT_DT_X.csv
- **json metadata**: https://data360files.worldbank.org/data360-data/metadata/WB_LPI_20/CT_DT_X.json
- **api template**: https://data360api.worldbank.org/data360/data?top=1000&skip=0&DATABASE_ID=WB_LPI_20&INDICATOR=CT_DT_X
- **dataset on Data360**: https://data360.worldbank.org/en/search?query=WB_LPI_20

## Definition

Time spent at the port of departure (defined by UNLOCODE) from arrival of full export container to completion of container loading on vessel
...
The export phase usually starts from a point when a container is received by the shipping line from the shipper until it is loaded on a vessel at the port of loading (port of origin). 
The container export dwell time is calculated as the time between the arrival of a full container to be exported to the origin port until the completion of container loading on vessel and its departure from port facilities. 
