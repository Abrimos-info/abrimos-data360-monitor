# Reference dwell time at port of arrival (LLDC)
...
- **csv**: https://data360files.worldbank.org/data360-data/data/WB_LPI_20/DT_REF_POA.csv
- **json metadata**: https://data360files.worldbank.org/data360-data/metadata/WB_LPI_20/DT_REF_POA.json
- **api template**: https://data360api.worldbank.org/data360/data?top=1000&skip=0&DATABASE_ID=WB_LPI_20&INDICATOR=DT_REF_POA
- **dataset on Data360**: https://data360.worldbank.org/en/search?query=WB_LPI_20

## Definition

Average import dwell time at the economy of the port of entry that is used most frequently for LLDC-destined cargo, calculated using only the containers destined to the coastal economy.
...
Import dwell time is measured from the discharge of a full container from the vessel at the port of discharge to the time the container exits the port premises by truck or rail. Reference container import dwell time corresponds to container import dwell time at identified port of arrival (entry) of LLDC for all shipments with destination in the coastal transit economy. Reference container import dwell time indicator is provided for comparison purposes with container import dwell time at the same economy for all shipments with final destination in the LLDC.
