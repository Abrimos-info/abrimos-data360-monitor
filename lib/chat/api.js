...
const { pickLang } = require('../views');

function resolveChatLang(req, body) {
  if (body.lang === 'en' || body.lang === 'es') return body.lang;
  return pickLang(req);
}
...
async function handleChat(req, res) {
  ...
  const lang = resolveChatLang(req, body);
  ...
  try {
    ...
    await runAgent({ messages: sanitized, emit, chatContext, focusCountries, focusChanged, alertId, lang });
  } catch (err) {
