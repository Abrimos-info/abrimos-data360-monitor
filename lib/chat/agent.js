...
const { loadGenerationContext } = require('./generation-context');
const alertsStore = require('../alerts-store');

const REPO_ROOT = path.resolve(__dirname, '..', '..');
const MAX_TURNS = parseInt(process.env.CHAT_MAX_TURNS || '8', 10);

function pickLocalized(field, lang = 'es') {
  if (!field) return '';
  if (typeof field === 'string') return field;
  return field[lang] || field.es || field.en || '';
}

function formatPublishedPieceBlock(alert, lang = 'es') {
  const title = pickLocalized(alert.title, lang) || alert.id;
  const lead = pickLocalized(alert.lead, lang);
  const story = pickLocalized(alert.story, lang) || pickLocalized(alert.body, lang);
  const claimIds = [...new Set((alert.claim_tokens || []).map((t) => t.claim_id).filter(Boolean))];
  const lines = [
    '## Pieza publicada',
    '',
    `- id: ${alert.id}`,
    `- tipo: ${alert.content_type || 'noticia'}`,
    `- dataset_id: ${alert.dataset_id || alert.indicator?.database_id || 'n/a'}`,
    `- titular: ${title}`,
  ];
  ...
  if (claimIds.length) {
    lines.push('', '### claim_ids de la pieza', '', ...claimIds.map((id) => `- ${id}`));
  }
  if (alert.content_type === 'reportaje' && Array.isArray(alert.noticia_ids) && alert.noticia_ids.length) {
    lines.push('', '### noticia_ids', '', ...alert.noticia_ids.map((id) => `- ${id}`));
  }
  return lines.join('\n');
}

function uiLanguageBlock(lang) {
  if (lang === 'en') {
    return '\n\n## UI language\n\nThe reader selected **English**. Respond in English. When citing the published piece, use its English title, lead, and story.';
  }
  return '\n\n## Idioma de la UI\n\nEl lector eligió **español**. Respondé en español. Al citar la pieza publicada, usá título, lead y story en español.';
}

function loadSystemPrompt(focusCountries, focusChanged, alertContext, lang = 'es') {
  const promptsDir = path.join(REPO_ROOT, 'lib', 'prompts');
  let base;
  if (alertContext) {
    base = fs.readFileSync(path.join(promptsDir, 'chat-scoped-system.md'), 'utf8');
    base = appendFocusToSystemPrompt(base, focusCountries, focusChanged);
    base += `\n\n${formatPublishedPieceBlock(alertContext, lang)}`;
    const gen = loadGenerationContext(alertContext);
    if (gen?.markdown) {
      base += '\n\n## Contexto de generación (pipeline Data360)\n\n';
      base += `_Fuente en disco: ${gen.source}${gen.truncated ? ' (truncado)' : ''}_\n\n`;
      base += gen.markdown;
    } else {
      base += '\n\n## Contexto de generación (pipeline Data360)\n\n';
      base += '_No hay archivo de contexto omnibus en disco para esta pieza. '
        + 'Respondé solo con el texto publicado y tools; no inventes series ni candidatos del pipeline._\n';
    }
  } else {
    base = fs.readFileSync(path.join(promptsDir, 'chat-system.md'), 'utf8');
    base = appendFocusToSystemPrompt(base, focusCountries, focusChanged);
  }
  base += uiLanguageBlock(lang);
  return base;
}
...
async function runAgent({ messages, emit, chatContext = {}, focusCountries = null, focusChanged = false, alertId = null, lang = 'es' }) {
  const tools = getToolDefinitions();
  const focus = normalizeFocusCountries(focusCountries);
  const alertContext = alertId ? alertsStore.getAlertById(alertId) : null;
  const system = loadSystemPrompt(focus, focusChanged === true, alertContext, lang);
...
module.exports = {
  runAgent,
  MAX_TURNS,
  wantsNewsToolsRequired,
  looksLikeFakeToolMarkdown,
  loadSystemPrompt,
  formatPublishedPieceBlock,
};
