...
function loadSystemPrompt(focusCountries, focusChanged) {
  const base = fs.readFileSync(path.join(REPO_ROOT, 'lib', 'prompts', 'chat-system.md'), 'utf8');
  return appendFocusToSystemPrompt(base, focusCountries, focusChanged);
}
...
async function runAgent({ messages, emit, chatContext = {}, focusCountries = null, focusChanged = false }) {
  const tools = getToolDefinitions();
  const focus = normalizeFocusCountries(focusCountries);
  const system = loadSystemPrompt(focus, focusChanged === true);
