...
const DATA360_INDICATOR_BASE = 'https://data360.worldbank.org/en/int/indicators';
const DATA360_DATASET_BASE = 'https://data360.worldbank.org/en/int/dataset';

function indicatorPageUrl(idno, databaseId) {
  if (!idno) return null;
  const db = databaseId || databaseIdFor(idno);
  return `${DATA360_INDICATOR_BASE}/${encodeURIComponent(idno)}`;
}

function datasetPageUrl(databaseId) {
  if (!databaseId) return null;
  return `${DATA360_DATASET_BASE}/${encodeURIComponent(databaseId)}`;
}
...
function trimAlertForLlm(alert) {
  ...
  return {
    ...
    citizen_es: alert.citizen_es || alert.narrative_citizen?.es?.slice(0, 220),
    citizen_en: alert.citizen_en || alert.narrative_citizen?.en?.slice(0, 220),
    journalist_es: alert.narrative_journalist?.es?.slice(0, 220),
    journalist_en: alert.narrative_journalist?.en?.slice(0, 220),
