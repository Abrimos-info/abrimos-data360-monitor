...
/** Heuristic themes for dynamic-tier idnos without explicit mapping. */
function themesForIndicatorOrFallback(idno) {
  const explicit = themesForIndicator(idno);
  if (explicit.length) return explicit;
  const upper = String(idno || '').toUpperCase();
  if (/PIP|POV|POOR|HEAD/.test(upper)) return ['POVERTY', 'WB_695_POVERTY'];
  if (/PFI|ROL|WGI|CORR/.test(upper)) return ['CORRUPTION'];
  if (/TRADE|BOP|KNOMAD|MIG/.test(upper)) return ['WB_698_TRADE'];
  if (/EMP|LABOR|UEM/.test(upper)) return ['WB_2748_EMPLOYMENT'];
  return PRIMARY_ANNUAL_GDELT_THEMES.slice(0, 2);
}
