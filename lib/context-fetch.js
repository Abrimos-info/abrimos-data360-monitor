...
const data360 = require('./data360-client');
const { dataDictUrl, metadataJsonUrl } = data360;
const {
  csvSnapshotPath,
  metaSnapshotPath,
  loadEtag,
...
async function fetchAndCacheMetadata(indicatorsDir, idno, db, label, force) {
  ...
  const md = await data360.getMetadata(idno);
...
async function downloadCsvSnapshot(snapshotsDir, entry, opts = {}) {
  ...
  const getCsvFn = opts.getCsv || data360.getCsv;
  const csvUrlFn = opts.csvUrl || data360.csvUrl;
...
async function downloadDataDict(snapshotsDir, entry) {
  ...
  const res = await data360.getCsv(url);
  if (res.status === 200 && res.body) {
    fs.mkdirSync(snapshotsDir, { recursive: true });
...
async function downloadMetadataJson(snapshotsDir, entry) {
  ...
  const data = await data360.getJson(url);
  if (data) {
    fs.mkdirSync(snapshotsDir, { recursive: true });
...
async function fetchObservationsForCountry(country, entry) {
  ...
  const data = await data360.getData(database_id, idno, country, { top: 5000 });
  return (data.value || [])
    ...
    .map((o) => ({
      ...
      comp_breakdown_1: o.COMP_BREAKDOWN_1 || '_Z',
      comp_breakdown_2: o.COMP_BREAKDOWN_2 || '_Z',
      comp_breakdown_3: o.COMP_BREAKDOWN_3 || '_Z',
      sex: o.SEX || '_T',
      age: o.AGE || '_T',
      urbanisation: o.URBANISATION || '_Z',
    }))
    .filter((r) => r.value !== null && r.value !== undefined && r.value !== '')
    .sort((a, b) => a.time_period.localeCompare(b.time_period));
}

const CONTEXT_CSV_HEADERS = [
  'indicator',
  'time_period',
  'value',
  'unit_measure',
...
async function refreshContextForIndicator(contextDir, entry) {
  ...
  const headers = CONTEXT_CSV_HEADERS;
...
async function ensureIndicatorMetadata(indicatorsDir, snapshotsDir, entries, opts = {}) {
  const { forceMetadata = false } = opts;
  const results = [];

  for (const entry of entries) {
    process.stdout.write(`  metadata ${entry.idno} ... `);
    try {
      ...
      await downloadDataDict(snapshotsDir, entry).catch((e) => {
        console.warn(`  dict ${entry.idno}: ${e.message}`);
      });
      await downloadMetadataJson(snapshotsDir, entry).catch((e) => {
        console.warn(`  meta ${entry.idno}: ${e.message}`);
      });
      process.stdout.write('ok\n');
      results.push({ idno: entry.idno, ok: true });
    } catch (e) {
...
async function refreshContextForIndicators(contextDir, indicatorsDir, snapshotsDir, entries, opts = {}) {
  const { forceMetadata = false, skipMetadata = false } = opts;
  const results = [];
...
function entriesNeedingMetadata(watchlist, indicatorsDir, changedIds, force) {
  const seen = new Set();
  const out = [];
  for (const entry of watchlist) {
    if (!entry?.idno || seen.has(entry.idno)) continue;
    const mdPath = path.join(indicatorsDir, `${entry.idno}.md`);
    const needs = force || changedIds.has(entry.idno) || !fs.existsSync(mdPath);
    if (needs) {
      seen.add(entry.idno);
      out.push(entry);
    }
