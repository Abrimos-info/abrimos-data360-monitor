...
const { latestDetectedAt, formatLastUpdate, headlineDateLabel } = require('./alert-display');
...
function pageLocals(req, extra = {}) {
  ...
  const monitorHomeUrl = `/${countrySlug(selectedCountryIso)}`;
  return {
    lang,
    langMode,
    ...
    selectedCountryIso: extra.countryIso || selectedCountryIso,
    monitorHomeUrl: extra.countryIso ? `/${countrySlug(extra.countryIso)}` : monitorHomeUrl,
    indicatorSearchUrl,
    indicatorUrl,
...
function pickLead(alert, lang) {
  ...
  if ((alert.content_type || 'noticia') === 'reportaje') return '';
...
async function indicatorsHubPage(req, res) {
  ...
  for (const [key, list] of Object.entries(tiersRaw)) {
    tiers[key] = enrichIndicatorList(list, lang);
  }
  const recentIndicators = enrichIndicatorList(recentUpdatedIndicators(16), lang);
...
async function indicatorDetailPage(req, res, idno) {
  if (!idno || !indicatorKnown(idno)) return notFoundPage(res);
  const lang = pickLang(req);
...
async function routePage(req, res) {
  ...
  if (segments.length === 1) {
    ...
    if (segments[0] === 'indicadores' || segments[0] === 'indicators') {
      return indicatorsHubPage(req, res);
    }
  ...
  if (segments.length === 2 && segments[0] === 'indicador') {
    return indicatorDetailPage(req, res, decodeURIComponent(segments[1]));
  }
