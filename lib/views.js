...
const { indicatorSearchUrl, indicatorUrl, datasetSearchUrl, resolvePublicDatasetUrl } = require('./data360-urls');
const {
...
function pickLang(req) {
  ...
  const fromCookie = parseCookie(req, 'd360_lang');
...
function writeHtml(res, html, opts = {}) {
  ...
  if (opts.countryIso && DEMO_COUNTRIES.includes(opts.countryIso)) {
    cookies.push(`d360_country=${opts.countryIso}; Path=/; Max-Age=31536000; SameSite=Lax`);
  }
  if (opts.lang === 'es' || opts.lang === 'en') {
    cookies.push(`d360_lang=${opts.lang}; Path=/; Max-Age=31536000; SameSite=Lax`);
  }
...
/** Reportajes on the frontpage keyed by id — for client-side PCN hydration (pcn-claims.js). */
function frontpageClaimAlerts(heroReportaje, headlineItems) {
  const map = {};
  if (heroReportaje?.id) map[heroReportaje.id] = heroReportaje;
  for (const item of headlineItems || []) {
    if (item.kind === 'reportaje' && item.alert?.id) {
      map[item.alert.id] = item.alert;
    }
  }
  return map;
}
...
async function frontpagePage(req, res, countrySlugParam) {
  ...
  const html = getTemplate('frontpage')(pageLocals(req, {
    ...
    heroReportaje,
    headlineItems,
    frontpageClaimAlerts: frontpageClaimAlerts(heroReportaje, headlineItems),
    tickerIndicators,
    editionLabel: editionText,
  ...
  writeHtml(res, html, { countryIso: iso, lang });
}
...
async function legacyDashboardPage(req, res, countryIso) {
  ...
  writeHtml(res, html, { lang: pickLang(req) });
}
...
async function alertPage(req, res, parsed) {
  ...
  writeHtml(res, html, { countryIso: parsed.iso, lang });
}
...
async function newsletterEditionPage(req, res, dateIso) {
  ...
  const { enrichEditionFromAlerts } = require('./newsletter/enrich-edition');
  const { buildDisplayEdition } = require('./newsletter/render-edition');
  const alertsStore = require('./alerts-store');
  const lang = pickLang(req);
  const rawEdition = loadEdition('lac', dateIso);
  if (!rawEdition) return notFoundPage(req, res);
  const edition = enrichEditionFromAlerts(rawEdition, alertsStore.getAlerts());
  const display = buildDisplayEdition(edition, alertsStore, lang);
...
function notFoundPage(req, res) {
  ...
  try {
    ...
    writeHtml(res, html, { lang, status: 404 });
    return;
  } catch (_) {
    /* fallback */
  }
  writeHtml(res, '<!doctype html><meta charset="utf-8"><title>404</title><h1>No encontrado</h1>', { lang, status: 404 });
}
