...
/** Substitute pipe inside markers while markdown table parser runs (GFM treats | as column sep). */
const CLAIM_PIPE_SUB = '\u0001';
...
function shieldClaimMarkersForMarkdown(text) {
  ...
  return normalizeClaimMarkerText(text).replace(
    CLAIM_MARKER_RE,
    (_, id, val) => `{{claim:${id}${CLAIM_PIPE_SUB}${val}}}`,
  );
}
