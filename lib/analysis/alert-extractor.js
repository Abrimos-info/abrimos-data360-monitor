...
function sanitizeReportajeItem(item, sourceNoticias = []) {
  ...
  const countries = [...new Set(sourceNoticias.flatMap((n) => n.countries || []))];
  ...
  const csvLinks = new Set(Array.isArray(item.verification_trace.csv_links) ? item.verification_trace.csv_links : []);
  if (item.verification_trace.csv_link) {
    csvLinks.add(item.verification_trace.csv_link);
    delete item.verification_trace.csv_link;
  }
  for (const n of sourceNoticias) {
    if (n.verification_trace?.csv_link) csvLinks.add(n.verification_trace.csv_link);
  }
