...
const NOTICIA_TOP_LEVEL = new Set([
  'content_type', 'id', 'title', 'lead', 'story', 'country', 'dataset_id',
...
const { ensureBilingual } = require('./normalizers');
const { normalizeCountry } = require('./country-utils');

function resolveCountryField(item) {
  if (!item || typeof item !== 'object') return null;
  if (item.country) return normalizeCountry(item.country);
  if (Array.isArray(item.countries) && item.countries.length) {
    return normalizeCountry(item.countries[0]);
  }
  return null;
}

function normalizeIndicatorIdList(value) {
  if (!Array.isArray(value)) return [];
  return [...new Set(value
    .map((entry) => {
      if (typeof entry === 'string') return entry.trim();
      if (entry && typeof entry === 'object') {
        return String(entry.idno || entry.code || '').trim();
      }
...
function sanitizeNoticiaItem(item) {
  ...
  item.country = resolveCountryField(item);
...
function sanitizeReportajeItem(item, sourceNoticias = []) {
  ...
  const sharedCountry = sourceNoticias.map((n) => normalizeCountry(n.country)).find(Boolean);
  const datasetId = item.dataset_id || sourceNoticias[0]?.dataset_id;

  item.dataset_id = datasetId;
  const fromLlm = normalizeIndicatorIdList(item.indicators);
  if (idnos.length >= 2) {
    item.indicators = idnos;
  } else if (fromLlm.length >= 2) {
    item.indicators = fromLlm;
  } else {
    item.indicators = idnos.length ? idnos : fromLlm;
  }
  if (!Array.isArray(item.noticia_ids) || !item.noticia_ids.length) {
    item.noticia_ids = noticiaIds;
  }
  if (!item.country) {
    item.country = sharedCountry || resolveCountryField(item);
  } else {
    item.country = normalizeCountry(item.country);
  }
  delete item.countries;
