...
function validateCountryField(item, opts = {}) {
  ...
  for (const token of item.claim_tokens || []) {
    ...
    if (!meta) continue;
    const claimCountry = normalizeCountry(meta.refArea);
    if (claimCountry === country) continue;
    // Regional LAC comparison: claims from other demo countries are allowed in story.
    if (isValidDemoCountry(claimCountry)) continue;
    failures.push({
      check: 'Q-country',
