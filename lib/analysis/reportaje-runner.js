'use strict';

const crypto = require('crypto');
const fs = require('fs');
const path = require('path');

const ai = require('../ai-client');
const { validateItem } = require('./quality-validator');
const { iterateFencedJson, coerceTypes, sanitizeReportajeItem, resolveClaims } = require('./alert-extractor');
const { buildNewsSectionForDataset } = require('../news');
const { COUNTRIES } = require('../data-loader');
const { annotateReportajeClaims } = require('../pcn-annotate');
const { lookupObservationByClaimId, clearCache } = require('../pcn-verify');
const { buildLlmDebug, llmStep } = require('./llm-debug');
const { datasetSearchUrl } = require('../data360-urls');
const { logTiming } = require('../timing');

const REPO_ROOT = path.resolve(__dirname, '..', '..');
const ALERTS_DIR = path.join(REPO_ROOT, 'data', 'alerts');

function reportajeFingerprint(noticias) {
  const parts = noticias
    .map((n) => `${n.id}|${n.quality_status || 'accepted'}`)
    .sort();
  return crypto.createHash('sha256').update(parts.join('\n')).digest('hex').slice(0, 16);
}

function loadCachedReportaje(datasetId) {
  const filepath = path.join(ALERTS_DIR, `reportaje_${datasetId}.json`);
  if (!fs.existsSync(filepath)) return null;
  try {
    return JSON.parse(fs.readFileSync(filepath, 'utf8'));
  } catch (_) {
    return null;
  }
}

function groupNoticiasByDatasetAndCountry(noticias, { minNoticias = 1 } = {}) {
  const groups = new Map();
  for (const n of noticias) {
    if (n.content_type !== 'noticia') continue;
    const country = String(n.country || '').toUpperCase();
    if (!country) continue;
    const key = `${n.dataset_id}::${country}`;
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(n);
  }
  for (const [key, items] of groups) {
    if (items.length < minNoticias) groups.delete(key);
  }
  return groups;
}

function noticiasEligibleForReportaje(noticias) {
  return noticias.filter((n) => {
    if (n.content_type !== 'noticia') return false;
    const status = n.quality_status || 'accepted';
    if (status === 'rejected') {
      console.warn(`[reportaje-runner] skipping noticia ${n.id} (quality_status=${status})`);
      return false;
    }
    if (status === 'accepted') return true;
    if (status === 'incomplete') {
      const verified = (n.claim_tokens || []).some((t) => t.pcn_status === 'verified');
      if (!verified) {
        console.warn(`[reportaje-runner] skipping noticia ${n.id} (quality_status=${status}, no verified claims)`);
        return false;
      }
      return true;
    }
    console.warn(`[reportaje-runner] skipping noticia ${n.id} (quality_status=${status})`);
    return false;
  });
}

function compactNoticiaForReportaje(n) {
  const verifiedClaims = (n.claim_tokens || []).filter(
    (t) => t.pcn_status === 'verified' || t.pcn_status == null
  );
  return {
    id: n.id,
    quality_status: n.quality_status || 'accepted',
    country: n.country,
    dataset_id: n.dataset_id,
    indicator: n.indicator ? {
      idno: n.indicator.idno,
      name: n.indicator.name,
    } : undefined,
    observation: n.observation,
    chart_series: n.chart_series,
    magnitude: n.magnitude,
    claim_tokens: verifiedClaims.length ? verifiedClaims : (n.claim_tokens || []),
  };
}

const { buildReportajePromptParts } = require('./prompt-builder');

function canonicalClaimValues(sourceNoticias) {
  const values = new Map();
  for (const n of sourceNoticias || []) {
    for (const t of n.claim_tokens || []) {
      if (!t?.claim_id) continue;
      if (t.pcn_status === 'verified' && t.value != null) {
        values.set(t.claim_id, String(t.value));
        continue;
      }
      if (values.has(t.claim_id)) continue;
      const row = lookupObservationByClaimId(t.claim_id, COUNTRIES);
      if (row?.value != null) values.set(t.claim_id, String(row.value));
    }
  }
  return values;
}

function syncReportajeClaimValues(item, sourceNoticias) {
  const canonical = canonicalClaimValues(sourceNoticias);

  if (!Array.isArray(item.claim_tokens) || !item.claim_tokens.length) {
    const storyText = `${item.story?.es || ''}${item.story?.en || ''}${item.lead?.es || ''}${item.lead?.en || ''}`;
    item.claim_tokens = resolveClaims(storyText);
  }
  for (const t of item.claim_tokens || []) {
    if (!t?.claim_id || canonical.has(t.claim_id)) continue;
    const row = lookupObservationByClaimId(t.claim_id, COUNTRIES);
    if (row?.value != null) canonical.set(t.claim_id, String(row.value));
  }
  if (!canonical.size) return item;

  for (const t of item.claim_tokens || []) {
    if (t?.claim_id && canonical.has(t.claim_id)) {
      t.value = canonical.get(t.claim_id);
    }
  }
  return item;
}

function buildReportajePrompt(datasetId, noticias, opts = {}) {
  const { system, task } = buildReportajePromptParts();

  const noticiasContext = noticias
    .map((n, i) => `### Noticia ${i + 1}\n\`\`\`json\n${JSON.stringify(compactNoticiaForReportaje(n), null, 2)}\n\`\`\``)
    .join('\n\n');

  const datasetUrl = datasetSearchUrl(datasetId);

  const allowedClaimIds = [
    ...new Set(noticias.flatMap((n) => (n.claim_tokens || []).map((t) => t.claim_id))),
  ];

  const idnos = [...new Set(noticias.map((n) => n.indicator?.idno).filter(Boolean))];
  const country = noticias[0]?.country;
  const asOfDay = opts.asOf ? String(opts.asOf).slice(0, 10) : null;
  const newsSection = buildNewsSectionForDataset(
    country ? [country] : COUNTRIES,
    idnos,
    {
      fromMonth: (opts.newsFrom || '2026-04-01').slice(0, 7),
      toMonth: (asOfDay || opts.newsTo || '2026-05-29').slice(0, 7),
      publishedTo: asOfDay,
      limitPerCountry: 6,
    },
  );

  const user = [
    task,
    '',
    '---',
    '',
    `## §7. Noticias generadas para dataset ${datasetId}`,
    '',
    '§7 incluye solo datos estructurados (`observation`, `chart_series`, `claim_tokens`). Redactá el reportaje desde estos valores; no hay story ni lead de las Noticias.',
    '',
    noticiasContext,
    '',
    '### allowed_claim_ids',
    'Reutilizá EXCLUSIVAMENTE estos `claim_id` heredados de las Noticias. Cualquier otro será rechazado por la validación automática.',
    '',
    ...allowedClaimIds.map((id) => `- ${id}`),
    '',
    `Dataset URL: ${datasetUrl}`,
  ];

  if (newsSection.any) {
    user.push('', '---', '', '## §8. Prensa reciente (contexto para citar en el reportaje)', '');
    user.push(...newsSection.lines);
    user.push('', 'Al citar prensa externa: fragmento textual entre comillas, medio, autor y enlace markdown [medio](url).');
  }

  const userContent = user.join('\n');

  return [
    { role: 'system', content: system },
    { role: 'user', content: userContent },
  ];
}

function parseReportajeBlock(llmText, sourceNoticias = []) {
  const results = [];
  for (const jsonText of iterateFencedJson(llmText, 'reportaje')) {
    try {
      const item = JSON.parse(jsonText);
      coerceTypes(item);
      sanitizeReportajeItem(item, sourceNoticias);
      results.push(item);
    } catch (_) {}
  }
  return results;
}

async function generateReportaje(datasetId, noticias, opts = {}) {
  const messages = buildReportajePrompt(datasetId, noticias, opts);

  if (opts.noLlm) {
    return { reportaje: null, skipped: true, reason: '--no-llm flag set' };
  }

  // Match the call pattern used in lib/analysis/runner.js: ai.complete(messages, { label })
  const llmT0 = Date.now();
  const llmResult = await ai.complete(messages, {
    label: `reportaje:${datasetId}`,
    model: process.env.AI_MODEL_REPORTAJE,
    effort: opts.effort,
  });
  logTiming('analysis', `reportaje:${datasetId} | llm`, Date.now() - llmT0);
  const items = parseReportajeBlock(llmResult.content, noticias);

  if (items.length === 0 && /```reportaje/.test(llmResult.content)) {
    fs.mkdirSync(ALERTS_DIR, { recursive: true });
    const rawPath = path.join(ALERTS_DIR, `reportaje_${datasetId}.raw.txt`);
    fs.writeFileSync(rawPath, llmResult.content, 'utf8');
    console.warn(`[reportaje-runner] ${datasetId}: LLM emitted a reportaje opener but parsing yielded 0 items — raw saved to ${path.relative(REPO_ROOT, rawPath)}`);
  }

  const contextClaimIds = new Set(
    noticias.flatMap((n) => (n.claim_tokens || []).map((t) => t.claim_id))
  );

  const llmDebug = buildLlmDebug([llmStep('reportaje', llmResult.stats)]);

  const expectedCountry = noticias[0]?.country;
  const valid = items.filter((item) => {
    syncReportajeClaimValues(item, noticias);
    annotateReportajeClaims(item, contextClaimIds, noticias);
    if (llmDebug) item.llm_debug = llmDebug;
    if (expectedCountry) item.country = expectedCountry;
    const { ok, failures } = validateItem(item, contextClaimIds, {
      expectedCountry,
      sourceNoticias: noticias,
    });
    if (!ok) {
      const failStr = failures.map((f) => `${f.check}: ${f.notes}`).join('; ');
      console.warn(`[reportaje-runner] ${datasetId}: reportaje failed validation — ${failStr}`);
      return false;
    }
    const rejectedClaims = (item.claim_tokens || []).filter((t) => t.pcn_status === 'rejected');
    if (rejectedClaims.length) {
      console.warn(`[reportaje-runner] ${datasetId}: ${rejectedClaims.length} PCN claim(s) rejected — reportaje kept`);
    }
    return true;
  });

  return { reportaje: valid[0] || null, raw: llmResult.content };
}

async function runReportajes(allNoticias, opts = {}) {
  const minNoticias = opts.minNoticias ?? 2;
  const eligible = noticiasEligibleForReportaje(allNoticias);
  const groups = groupNoticiasByDatasetAndCountry(eligible, { minNoticias });
  const reportajes = [];
  let skippedCache = 0;
  let failed = 0;

  for (const [groupKey, noticias] of groups) {
    const datasetId = noticias[0]?.dataset_id || groupKey.split('::')[0];
    const cacheKey = groupKey.replace('::', '_');
    const fingerprint = reportajeFingerprint(noticias);
    if (opts.changedOnly && !opts.noLlm) {
      const cached = loadCachedReportaje(cacheKey);
      if (cached && cached._source_fingerprint === fingerprint) {
        console.log(`[reportaje-runner] skipping ${datasetId} (unchanged source noticias, ${noticias.length} eligible)`);
        reportajes.push(cached);
        skippedCache += 1;
        continue;
      }
    }

    console.log(`[reportaje-runner] Generating Reportaje for dataset ${datasetId} (${noticias.length} noticias) ...`);
    const dsT0 = Date.now();
    try {
      const { reportaje } = await generateReportaje(datasetId, noticias, opts);
      if (reportaje) {
        reportaje.detected_at = reportaje.detected_at || (opts.asOf
          ? `${opts.asOf}T12:00:00.000Z`
          : new Date().toISOString());
        reportaje._source_fingerprint = fingerprint;
        reportajes.push(reportaje);

        fs.mkdirSync(ALERTS_DIR, { recursive: true });
        fs.writeFileSync(
          path.join(ALERTS_DIR, `reportaje_${cacheKey}.json`),
          JSON.stringify(reportaje, null, 2) + '\n',
          'utf8',
        );
      }
      logTiming('analysis', `reportaje:${datasetId} | total`, Date.now() - dsT0);
    } catch (err) {
      failed += 1;
      console.warn(`[reportaje-runner] ${datasetId} failed: ${err.message.split('\n')[0]}`);
    }
  }

  if (skippedCache) {
    console.log(`[reportaje-runner] ${skippedCache} reportaje(s) reused from cache (--changed-only)`);
  }
  if (failed) {
    console.warn(`[reportaje-runner] ${failed} reportaje(s) failed`);
  }

  return reportajes;
}

module.exports = {
  groupNoticiasByDatasetAndCountry,
  groupNoticiasByDataset: groupNoticiasByDatasetAndCountry,
  generateReportaje,
  runReportajes,
  parseReportajeBlock,
  buildReportajePrompt,
  compactNoticiaForReportaje,
  noticiasEligibleForReportaje,
  reportajeFingerprint,
  loadCachedReportaje,
  syncReportajeClaimValues,
  canonicalClaimValues,
};
