...
const { normalizeLlmNarrative } = require('./narrative-normalizer');
...
function buildLlmPrompt(context) {
  ...
  const task = [
    ...
    '',
    'Reglas:',
    '- Una entrada por candidate_id listado en `## Candidatos detectados` del contexto.',
    '- narrative_citizen: lenguaje llano, audiencia general, máximo 280 caracteres por idioma. En es, empezá con el verbo.',
    '- narrative_journalist: tono periodístico con z-score o mediana regional, máximo 280 caracteres por idioma.',
    '- claim_id: copiá exactamente el claim_id de `## Candidatos detectados` (16 caracteres hex).',
    '- No inventes datos que no estén en el contexto.',
    '',
    'Formato de respuesta:',
    '```json',
    '{ "narratives": { "<candidate_id>": { "narrative_citizen": { "es": "...", "en": "..." }, "narrative_journalist": { "es": "...", "en": "..." }, "claim_tokens": [] } }, "quality": [] }',
    '```',
    '',
    'CONTEXTO:',
    '```',
...
async function processOneIndicator(idno, candidates, allSeries, baselineSeries, opts = {}) {
  ...
  if (opts.noLlm) {
    for (const c of candidates) narratives[c.candidate_id] = deterministicNarrative(c);
  } else {
    try {
      const response = await ai.complete(messages, { label: `analysis:${idno}` });
      narratives = parseLlmResponse(response).narratives || {};
      for (const c of candidates) {
        narratives[c.candidate_id] = narratives[c.candidate_id]
          ? normalizeLlmNarrative(c, narratives[c.candidate_id], buildContextClaimIds([c]), deterministicNarrative, previousClaimId)
          : deterministicNarrative(c);
      }
    } catch (err) {
  ...
  for (const c of candidates) {
    ...
    const alert = enrichAlert({
      ...
      ...(n.claim_tokens?.length ? { claim_tokens: n.claim_tokens } : {}),
    });
    const { ok, failures } = validateAlert(alert, contextClaimIds);
    if (!ok) console.warn(`[analysis] validation issues for ${alert.id}: ${failures.map((f) => f.check).join(', ')}`);
