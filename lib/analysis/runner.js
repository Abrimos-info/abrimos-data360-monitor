...
function mergeReportajesIntoAggregate(reportajes) {
  if (!reportajes.length) return;
  const incomingIds = new Set(reportajes.map((r) => r.id).filter(Boolean));
  const kept = readAlertsAggregate().filter(
    (a) => a.content_type !== 'reportaje' || !incomingIds.has(a.id),
  );
  const aggregate = sortAlertsByDataDate([...kept, ...reportajes.map(enrichAlert)]);
  fs.writeFileSync(ALERTS_FILE, JSON.stringify(aggregate, null, 2), 'utf8');
}
  ...
  for (const country of COUNTRIES) {
    for (const tier of tiers) {
      ...
      for (const row of rows) {
        ...
        grouped[row.indicator].push(observationFromContextRow(row));
      }
...
function buildBaselineSeries() {
  ...
  for (const country of COUNTRIES) {
    ...
    for (const row of rows) {
      ...
      baselineSeries[country][row.indicator].push(observationFromContextRow(row));
    }
...
function previousClaimId(candidate) {
  ...
  if (!prev) return null;
  return computeClaimId(claimInputFromObservation(prev, {
...
async function processOneIndicator(idno, candidates, allSeries, baselineSeries, opts = {}) {
  ...
  if (opts.noLlm) {
    ...
    try {
      ...
      const classification = err.llmError || enrichLlmError(err, {
        transport: ai.resolveAnalysisProvider(),
      });
      if (opts.llmFailures) opts.llmFailures.record(classification, { idno });
      console.warn(formatLlmErrorLog(classification, { idno, label: `analysis:${idno}` }));
      console.warn(`[analysis] LLM failed for ${idno}: ${err.message.slice(0, 300)}`);
      noticias = [];
    }
  }
...
async function runAnalysis(opts = {}) {
  ...
  const replayDay = isReplayMode(opts);
  const llmFailures = opts.llmFailures || createLlmErrorAccumulator();
  const runOpts = { ...opts, llmFailures };
  const aggregateBeforeRun = readAlertsAggregate();
  const publishedIdnos = publishedIndicatorIdnos(aggregateBeforeRun);

  if (replayDay && runNoticias && !opts.only) {
    const indexMap = buildIndexMap(loadIndexRows());
    const strictBlob = strictBlobReplayEnabled(opts);
    const candidateCount = processedIdnos.length;
    processedIdnos = selectIdnosForReplayDay(processedIdnos, opts.asOf, {
      indexMap,
      published: publishedIdnos,
      strictBlob,
    });
    console.log(
      `[runner] replay ${opts.asOf}: ${processedIdnos.length} indicator(s) for this day `
      + `(${strictBlob ? 'strict blob date' : 'unpublished drip'}, `
      + `${publishedIdnos.size} already published, ${candidateCount} with candidates)`,
    );
  }

  if (limits.maxIndicators > 0 && processedIdnos.length > limits.maxIndicators) {
    console.log(`[runner] capping analysis to top ${limits.maxIndicators} indicator(s) by |z| (of ${processedIdnos.length})`);
    processedIdnos = processedIdnos.slice(0, limits.maxIndicators);
  }
  ...
  let skippedLimit = 0;
  const skipCached = !opts.only && !opts.noLlm && !replayDay
    && (opts.changedOnly || opts.appendAlerts || opts.skipCached);
  const applyNoticiaLimits = runNoticias && !opts.only && !opts.noLlm && isLimitEnabled(limits);
  let countryCounts = {};
  let runNoticiaCount = 0;
  if (applyNoticiaLimits) {
    const existing = replayDay
      ? noticiasForReplayDay(readAlertsAggregate(), opts.asOf)
      : readAlertsAggregate();
    countryCounts = countNoticiasByCountry(existing);
    runNoticiaCount = countNoticias(existing);
    const seedLabel = replayDay ? `seed ${runNoticiaCount} for ${String(opts.asOf).slice(0, 10)}` : `seed ${runNoticiaCount} in aggregate`;
    console.log(`[runner] noticia limits: ${formatLimitsLog(limits)} (${seedLabel})`);
  } else if (limits.maxIndicators > 0 || limits.maxPerCountry > 0 || limits.maxPerRun > 0) {
    console.log(`[runner] limits: ${formatLimitsLog(limits)}`);
  }
  if (runNoticias) {
    if (!opts.only && !opts.changedOnly && !opts.appendAlerts) fs.writeFileSync(ALERTS_FILE, '[]', 'utf8');
    for (const idno of processedIdnos) {
      const candidates = byIndicator.get(idno);
      if (skipCached && isUnchangedIndicator(ALERTS_DIR, idno, candidates)) {
        skippedUnchanged++;
        console.log(`[analysis] ${idno}: skipped (cached noticia, detection unchanged)`);
        if (!opts.only) {
      ...
      const primaryCountry = pickPrimaryCountry(candidates);
      if (applyNoticiaLimits) {
        const limitCheck = evaluateIndicatorLimit({
          primaryCountry,
          countryCounts,
          runCount: runNoticiaCount,
          limits,
        });
        if (limitCheck.skip) {
          skippedLimit++;
          console.log(`[analysis] ${idno}: skipped (${limitCheck.reason}: ${limitCheck.detail})`);
          results.push({ idno, alerts: [], skippedLimit: true });
          if (limitCheck.reason === 'run_cap') break;
          continue;
        }
      }
      if (opts.onProgress) opts.onProgress({ phase: 'indicator', idno, count: candidates.length });
      const result = await processOneIndicator(idno, candidates, allSeries, baselineSeries, runOpts);
      if (applyNoticiaLimits && result.alerts?.length) {
        runNoticiaCount += recordNoticias(countryCounts, result.alerts, { primaryCountry });
      }
      results.push(result);
    }
    if (skippedUnchanged) {
      console.log(`[analysis] ${skippedUnchanged} indicator(s) skipped (cached)`);
    }
    if (skippedLimit) {
      console.log(`[analysis] ${skippedLimit} indicator(s) skipped (noticia limits)`);
    }
  ...
  if (runReportajesPhase) {
    ...
    reportajes = await runReportajes(allNoticias, {
      noLlm: opts.noLlm,
      changedOnly: !replayDay && (opts.changedOnly || opts.appendAlerts || opts.skipCached),
      asOf: opts.asOf,
      effort: opts.effort,
    });
    ...
    if (reportajes.length > 0) {
      mergeReportajesIntoAggregate(reportajes);
    }
  }

  timer.end('pipeline', `${allAlerts.length} noticias, ${reportajes.length} reportajes`);

  for (const line of llmFailures.summaryLines()) {
    console.warn(line);
  }
  ...
  return {
    ...
    indicatorsSkippedLimit: skippedLimit,
    alertCount: allAlerts.length,
    reportajeCount: reportajes.length,
    llmFailures: llmFailures.summaryLines(),
    results,
  };
}
