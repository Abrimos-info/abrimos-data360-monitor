...
async function runAnalysis(opts = {}) {
  ...
  let skippedLimit = 0;
  const skipCached = !opts.only && !opts.noLlm
  ...
  if (applyNoticiaLimits) {
    const existing = readAlertsAggregate();
    countryCounts = countNoticiasByCountry(existing);
    runNoticiaCount = countNoticias(existing);
    console.log(`[runner] noticia limits: ${formatLimitsLog(limits)} (seed ${runNoticiaCount} in aggregate)`);
  ...
  if (runReportajesPhase) {
    ...
    reportajes = await runReportajes(allNoticias, {
      noLlm: opts.noLlm,
      changedOnly: opts.changedOnly || opts.appendAlerts || opts.skipCached,
      asOf: opts.asOf,
      effort: opts.effort,
    });
