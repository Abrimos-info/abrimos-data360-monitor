...
function buildLlmPrompt(context, idno, { esOnly = NOTICIA_TRANSLATE } = {}) {
  ...
  const user = [
    fs.readFileSync(path.join(promptsDir, taskFile), 'utf8'),
    '',
    '---',
    '',
    context,
...
async function processOneIndicator(idno, candidates, allSeries, baselineSeries, opts = {}) {
  ...
  if (opts.noLlm) {
    ...
    try {
      ...
      if (noticias.length === 0 && /```noticia/.test(response)) {
        ensureDir(ALERTS_DIR);
  ...
  for (const item of noticias) {
    ...
    if (opts.translate !== false && NOTICIA_TRANSLATE && !opts.noLlm) {
      try {
        const trT0 = Date.now();
        enriched = sanitizeNoticiaItem(await translateNoticia(enriched, contextClaimIds, opts));
    ...
    enriched.detected_at = new Date().toISOString();
...
async function runAnalysis(opts = {}) {
  ...
  if (runNoticias) {
    if (!opts.only && !opts.changedOnly) fs.writeFileSync(ALERTS_FILE, '[]', 'utf8');
  ...
  if (runReportajesPhase) {
    ...
    const rpT0 = Date.now();
    reportajes = await runReportajes(allNoticias, { noLlm: opts.noLlm });
