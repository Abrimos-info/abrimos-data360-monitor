...
function loadAllSeries() {
  ...
  for (const country of COUNTRIES) {
    for (const tier of ['annual', 'pulse', 'forecast', 'dynamic']) {
...
async function runAnalysis(opts = {}) {
  ...
  // Phase 2: Reportajes (one per dataset with ≥2 Noticias)
  // Skip when --only is set since we don't have a full noticia set.
  let reportajes = [];
  if (!opts.only) {
