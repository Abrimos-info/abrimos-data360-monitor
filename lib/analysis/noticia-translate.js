...
async function translateNoticia(noticia, contextClaimIds, opts = {}) {
  if (opts.noTranslate) return { noticia, stats: null };
  ...
  const parsed = parseTranslateBlock(llmResult.content);
  if (!parsed) {
    console.warn(`[noticia-translate] ${label}: no parseable noticia-translate block`);
    return { noticia, stats: llmResult.stats };
  }
  return { noticia: mergeTranslation(noticia, parsed), stats: llmResult.stats };
}
