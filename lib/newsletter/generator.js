...
const CANONICAL_CTA_LABEL = {
  es: 'Ver la edición completa →',
  en: 'See the full edition →',
};

function canonicalEditionUrl(dateIso) {
  return `/newsletter/lac/${dateIso}`;
}

function canonicalCta(dateIso) {
  return {
    label: { ...CANONICAL_CTA_LABEL },
    url: canonicalEditionUrl(dateIso),
  };
}

/** Force edition-level CTA and footer links regardless of LLM output. */
function applyCanonicalEditionUrls(edition, dateIso) {
  if (!edition || !dateIso) return edition;
  edition.cta = canonicalCta(dateIso);
  if (!edition.footer) edition.footer = {};
  edition.footer.methodology_link = '/metodologia';
  return edition;
}
...
async function generateNewsletterEdition({ dateIso, alerts, noLlm = false, effort = null } = {}) {
  ...
  if (noLlm) {
    ...
    applyCanonicalEditionUrls(dry, dateIso);
  ...
  applyCanonicalEditionUrls(edition, dateIso);
  const { enrichEditionFromAlerts } = require('./enrich-edition');
