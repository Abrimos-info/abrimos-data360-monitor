...
const PROVIDER = process.env.AI_PROVIDER || process.env.AI_MODE || 'claude-code';
...
function logTokenUsage(modelKey, inputTokens, outputTokens, label = '', durationMs = null) {
  ...
  console.log(`[AI-COST] ${modelKey} | in: ${inputTokens} | out: ${outputTokens}${timeStr}${costStr}${tag}`);
}
...
async function completeVllm(messages, options = {}) {
  ...
  const model = options.model || process.env.AI_MODEL || VLLM_MODEL;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          ...
          `  url: ${VLLM_URL}/chat/completions | model: ${model}\n` +
      ...
      logTokenUsage(model, inputTokens, outputTokens, label, durationMs);
      return output;
    } catch (err) {
      lastErr = err.name === 'AbortError'
        ? new Error(`[vllm] Timeout after ${TIMEOUT_MS / 1000}s on ${VLLM_URL}`)
        : err;
      if (attempt < maxRetries && isRetryableNetworkError(lastErr)) {
        const delay = 2000 * (attempt + 1);
        console.warn(`[AI-WARN] ${label || 'vllm'} network error — retry ${attempt + 1}/${maxRetries} in ${delay}ms`);
  ...
  throw lastErr || new Error('[vllm] request failed');
}
  ...
  return new Promise((resolve, reject) => {
    ...
    proc = execFile(
      ...
      (err, stdout, stderr) => {
        ...
        logTokenUsage(modelKey, inputTokens, outputTokens, label, Date.now() - t0);
        resolve(stdoutTrim);
      }
    );
  ...
  if (PROVIDER === 'claude-code') {
    try {
      ...
      if (process.env.AI_FALLBACK_VLLM_ON_CLAUDE_QUOTA === 'true' && isClaudeQuotaMessage(err.message)) {
        ...
        return await completeVllm(messages, { ...options, model: modelOverride });
      }
      throw err;
    }
  }
...
const CHAT_PROVIDER = process.env.CHAT_AI_PROVIDER || 'vllm';

function getChatModelInfo() {
  if (CHAT_PROVIDER === 'claude-code') {
    return { provider: 'claude-code', model: CLAUDE_MODEL };
  }
  return { provider: 'vllm', model: VLLM_MODEL };
}
...
async function chatCompleteVllm(messages, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`[vllm-chat] HTTP ${res.status}: ${errBody.slice(0, 800)}`);
    }
    ...
    logTokenUsage(VLLM_MODEL, inputTokens, outputTokens, label, durationMs);
    return {
...
async function chatCompleteClaude(messages, options = {}) {
  ...
  const toolsHint = options.tools?.length
    ...
    : '';
  const response = await completeClaudeCode([
    { role: 'user', content: flat + toolsHint },
  ], { label: options.label || 'chat' });

  const durationMs = Date.now() - t0;
  const outputTokens = estimateTokens(response);
  const inputTokens = estimateTokens(flat);
  const tokensPerSecond = durationMs > 0 ? outputTokens / (durationMs / 1000) : 0;
  const baseStats = {
...
async function chatComplete(messages, options = {}) {
  if (!ENABLED) throw new Error('AI_ENABLED is false');
  if (CHAT_PROVIDER === 'claude-code') return chatCompleteClaude(messages, options);
