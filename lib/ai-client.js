...
const { enrichLlmError } = require('./llm-error');
...
const CLAUDE_EFFORT = (process.env.CLAUDE_EFFORT || '').trim().toLowerCase();
...
function getAnalysisModelInfo() {
  ...
  if (provider === 'claude-code') {
    const viaApi = Boolean(ANTHROPIC_API_KEY);
    return {
      provider: 'claude-code',
      providerLabel: viaApi ? 'Anthropic' : 'Claude',
      model: viaApi ? anthropicModelForAlias(CLAUDE_MODEL) : CLAUDE_MODEL,
    };
  }
  if (provider === 'nvidia') {
    return {
...
function logAnalysisLlm(scope = 'AI-ANALYSIS') {
  ...
  const transport = info.provider === 'claude-code' && ANTHROPIC_API_KEY ? 'Anthropic API' : null;
  console.log(`[${scope}] LLM: ${info.providerLabel} | ${info.model}${transport ? ` (${transport})` : ''}`);
  return info;
}
...
function anthropicModelForAlias(alias) {
  if (process.env.ANTHROPIC_MODEL) return process.env.ANTHROPIC_MODEL;
  const m = String(alias || '').toLowerCase();
  if (m.includes('opus')) return process.env.ANTHROPIC_MODEL_OPUS || 'claude-opus-4-20250514';
  if (m.includes('haiku')) return process.env.ANTHROPIC_MODEL_HAIKU || 'claude-haiku-3-5-20241022';
  if (m.includes('sonnet')) return process.env.ANTHROPIC_MODEL_SONNET || 'claude-sonnet-4-20250514';
  return alias;
}

function usesAnthropicApi() {
  return Boolean(ANTHROPIC_API_KEY);
}

async function completeAnthropic(messages, options = {}) {
  if (!ANTHROPIC_API_KEY) {
    throw new Error('[anthropic] ANTHROPIC_API_KEY is required when AI_PROVIDER=claude-code');
  }
  ...
  const promptSizeKB = ((system.length + userContent.length) / 1024).toFixed(1);
  const timeoutMs = userContent.length / 1024 > 10 ? REPORT_TIMEOUT_MS : TIMEOUT_MS;
  const maxRetries = parseInt(process.env.AI_FETCH_RETRIES || '3', 10);
  const candidates = buildClaudeModelCandidates().map(anthropicModelForAlias);

  let lastErr;
  for (let i = 0; i < candidates.length; i++) {
    const model = candidates[i];
    const isLast = i === candidates.length - 1;
    logLlmCall('Anthropic', model, label, promptSizeKB, inputTokens);
      ...
      try {
        ...
        if (!res.ok) {
          const err = new Error(
            ...
            `  model: ${model} | prompt: ${promptSizeKB}KB\n` +
          ...
          if ((res.status === 429 || res.status === 529) && !isLast) {
            console.warn(`[AI-WARN] anthropic quota/rate on ${model}, trying ${candidates[i + 1]}`);
            lastErr = err;
            break;
          }
          throw err;
        ...
        if (!output) {
          throw new Error(`[anthropic] Empty response: ${bodyText.slice(0, 500)}`);
        }
        const usage = data.usage || {};
        const outputTokens = usage.output_tokens || estimateTokens(output);
        const resolvedInput = usage.input_tokens || inputTokens;
        const durationMs = Date.now() - t0;
        const modelKey = normalizeModelKey(`claude-${CLAUDE_MODEL}`);
        logTokenUsage(modelKey, resolvedInput, outputTokens, label, durationMs, 'Anthropic');
        return {
          content: output,
          stats: makeCompletionStats(
            { provider: 'claude-code', providerLabel: 'Anthropic' },
            model,
            durationMs,
            resolvedInput,
            outputTokens,
        ...
        lastErr = err.name === 'AbortError'
          ? new Error(`[anthropic] Timeout after ${timeoutMs / 1000}s`)
          : err;
        if (attempt < maxRetries && /fetch failed|ECONNRESET|ETIMEDOUT|ENOTFOUND|EAI_AGAIN/i.test(lastErr.message)) {
          await new Promise((r) => setTimeout(r, 2000 * (attempt + 1)));
          continue;
        }
        if (isClaudeQuotaMessage(lastErr.message) && !isLast) {
          console.warn(`[AI-WARN] anthropic quota on ${model}, trying ${candidates[i + 1]}`);
          break;
        }
        throw lastErr;
      }
...
async function completeClaude(messages, options = {}) {
  if (usesAnthropicApi()) return completeAnthropic(messages, options);
  return completeClaudeCode(messages, options);
}
...
function claudeTransport() {
  return ANTHROPIC_API_KEY ? 'anthropic_api' : 'claude_cli';
}

function rejectClaudeError(reject, message, meta = {}) {
  const err = new Error(message);
  enrichLlmError(err, { transport: claudeTransport(), ...meta });
  reject(err);
}

function resolveClaudeEffort(options = {}) {
  const raw = (options.effort || process.env.CLAUDE_EFFORT || '').trim().toLowerCase();
  if (!raw) return '';
  if (['low', 'medium', 'high'].includes(raw)) return raw;
  console.warn(`[AI-WARN] Ignoring invalid CLAUDE_EFFORT="${raw}" (expected low|medium|high)`);
  return '';
}

function runClaudeOnce(model, prompt, timeoutMs, inputTokens, promptSizeKB, label, effort = '') {
    ...
    proc = execFile(
      ...
      (err, stdout, stderr) => {
        ...
        if (err) {
          ...
          return rejectClaudeError(reject, detail, {
            stderr: stderrTrim,
            stdout: stdoutTrim,
            exitCode: err.code ?? err.signal,
          });
        }
        if (!stdoutTrim) {
          return rejectClaudeError(reject, `[claude-code] Empty response for model ${model}`, {
            stdout: stdoutTrim,
          });
        }
        if (isClaudeQuotaMessage(stdoutTrim)) {
          return rejectClaudeError(reject, `[claude-code] Quota reached: ${stdoutTrim.slice(0, 400)}`, {
            stdout: stdoutTrim,
          });
        }
...
async function completeClaudeCode(messages, options = {}) {
  ...
  const effort = resolveClaudeEffort(options);
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    console.log(`[AI-DEBUG] claude -p --model=${model}${effortFlag} | timeout: ${timeoutMs / 1000}s`);
    try {
      const result = await runClaudeOnce(model, prompt, timeoutMs, inputTokens, promptSizeKB, label, effort);
      return result;
    } catch (err) {
      lastErr = err;
...
async function complete(messages, options = {}) {
  ...
  if (provider === 'claude-code') {
    try {
      return await completeClaude(messages, opts);
    } catch (err) {
...
async function chatCompleteClaude(messages, options = {}) {
  ...
  const toolsHint = options.tools?.length
    ...
    : '';
  const claudeResult = await completeClaude([
