 ...
 *                             Free.
 *   AI_PROVIDER=nvidia        NVIDIA NIM (OpenAI-compatible chat/completions).
 *
...
const NVIDIA_MODEL = process.env.AI_MODEL_NVIDIA || process.env.AI_MODEL || 'moonshotai/kimi-k2.6';
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      lastErr = err.name === 'AbortError'
        ? new Error(`[${providerTag}] Timeout after ${TIMEOUT_MS / 1000}s on ${baseUrl}`)
        : err;
...
async function complete(messages, options = {}) {
  ...
  if (PROVIDER === 'nvidia') {
    return await completeNvidia(messages, { ...options, model: modelOverride });
  }
...
async function chatComplete(messages, options = {}) {
  ...
  if (CHAT_PROVIDER === 'nvidia') return chatCompleteNvidia(messages, options);
