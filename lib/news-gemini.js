...
const COUNTRY_MACRO_TOPICS = 'economía, inflación, pobreza, empleo, comercio exterior, deuda pública, gobernanza y corrupción';

function buildCountrySearchPrompt(country, { from, to, mediaHints = [], maxArticles = 8 } = {}) {
  const countryName = COUNTRY_GDELT[country] || country;
  const mediaLine = mediaHints.length
    ? `Medios de referencia en ${countryName} (orientación, no exclusivo): ${mediaHints.join(', ')}.`
    : '';

  return `Busca titulares recientes en prensa en español sobre temas macroeconómicos y de desarrollo en ${countryName}.
Temas: ${COUNTRY_MACRO_TOPICS}.
...
Máximo ${Math.max(1, Math.min(8, maxArticles))} artículos.`;
}
...
async function fetchNewsForCountry(country, opts = {}) {
  const t0 = Date.now();
  const from = opts.from || DEFAULT_FROM;
  const to = opts.to || DEFAULT_TO;
  const maxArticles = opts.maxArticles ?? opts.maxPerIndicator ?? 8;
  const mediaHints = countryMediaHints(country);
  const prompt = buildCountrySearchPrompt(country, { from, to, mediaHints, maxArticles });
  const fetchedAt = new Date().toISOString();
  const classifyCtx = {
    fetchedAt,
    from,
  ...
  try {
    ...
    ({ appended, accepted } = await saveGeminiArticles(
      country, null, articles, classifyCtx, opts, rawRecord,
    ));
    ...
    const rawPath = appendRawGeminiResponse(country, rawRecord, opts.rawDir);
    if (rawPath) {
      pipeLog(SCOPE, 'raw', { path: path.relative(REPO_ROOT, rawPath), country, mode: 'country-pool' });
    }
  }

  logTiming('news-gemini', `${country} pool`, Date.now() - t0, `${accepted} accepted / ${appended} saved`);
  return { fetched: rawRecord.articles_fetched, appended, accepted };
}
