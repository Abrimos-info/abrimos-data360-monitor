...
5. Completá `country` con el ISO3 del candidato protagonista (mayor |z|) únicamente. Los `claim_tokens` pueden incluir `claim_id` de **otros países LAC** del contexto para comparación regional (bloque 3); el lead y `observation` deben ser del protagonista.
