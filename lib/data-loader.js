...
const DEFAULT_DYNAMIC_WATCHLIST_PATH = path.join(REPO_ROOT, 'data', 'dynamic-watchlist.json');
...
function resolveDynamicWatchlistPath() {
  ...
  return DEFAULT_DYNAMIC_WATCHLIST_PATH;
}
...
function loadDynamicWatchlist() {
  const fp = resolveDynamicWatchlistPath();
  if (!fs.existsSync(fp)) return [];
  try {
    const watchlist = JSON.parse(fs.readFileSync(fp, 'utf8'));
...
function observationFromContextRow(row) {
  return {
    time_period: row.time_period,
    value: row.value,
    unit_measure: row.unit_measure,
    comp_breakdown_1: row.comp_breakdown_1 || '_Z',
    comp_breakdown_2: row.comp_breakdown_2 || '_Z',
    comp_breakdown_3: row.comp_breakdown_3 || '_Z',
    sex: row.sex || '_T',
    age: row.age || '_T',
    urbanisation: row.urbanisation || '_T',
  };
}
...
function groupSeriesByCountry(tier) {
  ...
  for (const country of COUNTRIES) {
    ...
    for (const row of rows) {
      ...
      series[country][idno].push(observationFromContextRow(row));
    }
  }
  return series;
}
...
module.exports = {
  ...
  INDICATORS_DIR,
  SNAPSHOTS_DIR,
  DEFAULT_DYNAMIC_WATCHLIST_PATH,
  resolveDynamicWatchlistPath,
  NOTICIA_TIERS,
  CONTEXT_TIERS,
