...
function renderClaimMarkers(text, alert, lang) {
  ...
  return normalizeClaimMarkerText(text).replace(CLAIM_MARKER_RE, (_, id, fallback) => {
    const token = findClaimToken(tokens, id, fallback);
    if (token) return markerDisplayText(fallback, token, lang);
    return fallback || id;
  });
}
...
function enrichBilingualField(field, alert, lang) {
  ...
  // Keep {{claim:…}} markers for client-side PCN marks (detail panel / alert pages).
...
function enrichAlert(alert) {
  ...
  // New Noticia/Reportaje bilingual fields. Render claim markers in place.
  if (enriched.title && typeof enriched.title === 'object') {
    enriched.title = enrichBilingualField(enriched.title, enriched, 'es');
  }
  if (enriched.lead && typeof enriched.lead === 'object') {
    enriched.lead = enrichBilingualField(enriched.lead, enriched, 'es');
  }
  if (enriched.story && typeof enriched.story === 'object') {
    enriched.story = enrichBilingualField(enriched.story, enriched, 'es');
  }
  if (enriched.body && typeof enriched.body === 'object') {
    enriched.body = enrichBilingualField(enriched.body, enriched, 'es');
  }

  // Legacy fields kept guarded for backward compatibility during transition.
...
function headlineDateLabel(alert, lang) {
  if (!alert || typeof alert !== 'object') return '';
  const lng = lang === 'en' ? 'en' : 'es';
  const period = alert.observation?.time_period;
  if (period) {
    if (/^\d{4}-\d{2}/.test(period) || /^\d{4}-Q\d/i.test(period)) {
      return alert.observation?.period_display?.[lng]
        || formatPeriodDisplay(period, lng);
    }
    if (/^\d{4}$/.test(period)) {
      const months = lng === 'en' ? MONTHS_EN : MONTHS_ES;
  ...
  return period ? formatPeriodDisplay(period, lng) || String(period) : '';
}
