...
function renderDumbbellChartSvg(payload, opts = {}) {
  ...
  for (const tv of ticks) {
    ...
    const label = Math.abs(tv) >= 1e9 ? `${(tv / 1e9).toFixed(1)}B`
      : Math.abs(tv) >= 1e6 ? `${(tv / 1e6).toFixed(1)}M`
      : Math.abs(tv) >= 1e3 ? `${(tv / 1e3).toFixed(1)}K`
      : tv.toFixed(Math.abs(tv) < 10 ? 2 : 0);
  ...
  rows.forEach((r, i) => {
    ...
    // End value label — placed away from the start dot to avoid overlap
    const valLabel = Math.abs(r.endValue) >= 1e9 ? `${(r.endValue / 1e9).toFixed(1)}B`
      : Math.abs(r.endValue) >= 1e6 ? `${(r.endValue / 1e6).toFixed(1)}M`
      : Math.abs(r.endValue) >= 1e3 ? `${(r.endValue / 1e3).toFixed(1)}K`
      : r.endValue.toFixed(Math.abs(r.endValue) < 10 ? 2 : 0);
    // Label: prefer outside the dumbbell, but fall back to right if left would overlap country label
    const wentDown = cx2 < cx1;
    const leftWouldOverlap = wentDown && (cx2 - 10) < labelW + 28;
    const labelRight = !wentDown || leftWouldOverlap;
    const valX = labelRight ? cx2 + 10 : cx2 - 10;
    const valAnchor = labelRight ? 'start' : 'end';
    svg += `<text class="d360-dumbbell__value" x="${valX}" y="${y + 4}" text-anchor="${valAnchor}">${escHtml(valLabel)}</text>`;
  });

  // Period legend bottom-left
  if (rows[0] && rows[0].startPeriod && rows[0].endPeriod) {
    const lx = labelW;
    const ly = h - 4;
    svg += `<circle cx="${lx + 5}" cy="${ly - 4}" r="4" fill="none" stroke="#888" stroke-width="1.2"/>`;
    svg += `<text class="d360-dumbbell__legend-text" x="${lx + 13}" y="${ly}">${escHtml(rows[0].startPeriod)}</text>`;
    svg += `<circle cx="${lx + 58}" cy="${ly - 4}" r="5" fill="#888"/>`;
    svg += `<text class="d360-dumbbell__legend-text" x="${lx + 66}" y="${ly}">${escHtml(rows[0].endPeriod)}</text>`;
  }
