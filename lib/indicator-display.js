...
const { formatPeriodDisplay } = require('./alert-display');
...
function formatDataPeriod(value, lang) {
  ...
  const periodLabel = formatPeriodDisplay(raw, lang);
...
function titleFromMetadata(idno, lang) {
  const FALLBACK_TITLES = {
    WJP_ROL_FAC_6: {
      es: 'WJP Rule of Law Index: Factor 6 - Cumplimiento normativo',
      en: 'WJP Rule of Law Index: Factor 6 - Regulatory Enforcement',
    },
  };
  const f = FALLBACK_TITLES[String(idno || '').trim().toUpperCase()];
  if (f) return (lang === 'en' ? f.en : f.es) || f.en || f.es;
  ...
  if (md) {
    ...
    if (m) {
      ...
      if (title && title !== idno) return title;
    }
  }
  const meta = loadIndicatorMetadataJson(idno);
  const sd = meta?.series_description || meta;
  const name = sd?.name;
  if (name && String(name).trim() && String(name).trim() !== idno) {
...
function resolveIndicatorName(entry, lang, wl, idx) {
  ...
  const fromMeta = titleFromMetadata(entry.idno, lang);
