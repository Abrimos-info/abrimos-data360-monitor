...
/** Find a local CSV row by PCN claim_id (searches all CONTEXT_TIERS per country). */
function lookupObservationByClaimId(claimId, refAreas = COUNTRIES) {
  if (!claimId || String(claimId).length !== 16) return null;
  for (const refArea of refAreas) {
    if (!COUNTRIES.includes(refArea)) continue;
    const obs = buildCountryIndex(refArea).get(`cid:${claimId}`);
    if (obs) return obs;
  }
  return null;
}
