...
const TIERS = ['annual', 'pulse', 'forecast'];
...
function buildCountryIndex(country) {
  ...
  for (const tier of TIERS) {
    ...
    for (const row of parseCsv(fp)) {
      const k = `${row.indicator}|${row.time_period}`;
      index.set(k, { ...row, country, tier });
    }
  }
...
function verifyClaim(claimId, expectedValue, indicator, refArea, timePeriod) {
  ...
  const obs = index.get(`${indicator}|${timePeriod}`);
  if (!obs) {
    return {
  ...
  const expectedId = computeClaimId({ indicator, country: refArea, time_period: timePeriod, unit_measure: obs.unit_measure || '' });
...
function verifyNarrativeClaims(tokens, claimMap) {
  ...
  for (const { claimId, value } of tokens) {
    ...
    const r = verifyClaim(claimId, value, meta.indicator, meta.refArea, meta.timePeriod);
