...
function primaryCountry(alert) {
  ...
  if (Array.isArray(alert.countries) && alert.countries.length) return alert.countries[0];
  if (Array.isArray(alert._countries) && alert._countries.length) return alert._countries[0];
  return 'ARG';
}
...
function alertTitleForSlug(alert) {
  ...
  if (alert.narrative_citizen?.es) return alert.narrative_citizen.es.slice(0, 60);
...
function countriesForAlert(alert) {
  ...
  return [...new Set(list.length ? list : [primaryCountry(alert)])];
}
...
function assignUniquePaths(alerts) {
  ...
  return alerts.map((alert) => {
    const paths = {};
    for (const iso of countriesForAlert(alert)) {
      paths[iso] = uniquePathForAlert(alert, iso, used);
    }
    const primaryIso = primaryCountry(alert);
    const path = paths[primaryIso] || Object.values(paths)[0];
